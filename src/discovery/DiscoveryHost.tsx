...
function cloneControls(controls: EngineControls): EngineControls {
  return {
    ...controls,
    target: [...controls.target] as Vector3,
  };
}

const EMPTY_OPTIMIZATION_MEMORY: OptimizationMemory = {
  decision: null,
  intent: null,
  navigation: null,
};
...
export default function DiscoveryHost({ mode }: { mode: EngineMode }) {
  ...
  const [autoRouteDeckBase, setAutoRouteDeckBase] = useState<EngineControls>(() => cloneControls(DEFAULT_MANUAL_CONTROLS));
  const [autoRouteDeck, setAutoRouteDeck] = useState<AutoRouteRecommendation[]>(() => evaluateAutoRoutes(DEFAULT_MANUAL_CONTROLS));
  const [selectedAutoRouteId, setSelectedAutoRouteId] = useState<string | null>(null);
  const [selectedNavigatorPresetId, setSelectedNavigatorPresetId] = useState<string | null>(findMatchingPresetId(DEFAULT_MANUAL_CONTROLS, presetList));
  ...
  const [lastOptimization, setLastOptimization] = useState<OptimizationMemory>(EMPTY_OPTIMIZATION_MEMORY);
  ...
  useEffect(() => {
    loadPersistedProductState().then((persisted) => {
      if (persisted) {
        const restoredManualControls = cloneControls(persisted.manualControls);
        setManualControls(restoredManualControls);
        setAutoRouteDeckBase(restoredManualControls);
        setAutoRouteDeck(evaluateAutoRoutes(restoredManualControls));
        setSelectedAutoRouteId(null);
        setSelectedNavigatorPresetId(persisted.selectedNavigatorPresetId);
        ...
        setLastOptimization(persisted.lastOptimization);
  ...
  const autoRoutes = autoRouteDeck;
  const winningAutoRoute = autoRoutes[0];
  const selectedAutoRoute = useMemo(
    () => autoRoutes.find((route) => route.id === selectedAutoRouteId) ?? null,
    [autoRoutes, selectedAutoRouteId],
  );
  const manualPracticalInputs = useMemo(() => inferPracticalInputsFromEngine(manualControls), [manualControls]);
    ...
    const leadingLabel =
      mode === "DECISION"
        ...
        : mode === "INTENT"
          ...
          : mode === "AUTOPILOT"
            ? selectedAutoRoute?.name ?? winningAutoRoute?.name ?? "Auto Route"
    ...
    const leadingTitle =
      mode === "DECISION"
        ...
        : mode === "INTENT"
          ? "Outcome"
          : mode === "AUTOPILOT"
            ? selectedAutoRoute ? "Applied Route" : "Top Route"
          : "Target Vector";
  ...
  const rebuildAutoRouteDeck = useCallback((controls: EngineControls) => {
    const snapshot = cloneControls(controls);
    setAutoRouteDeckBase(snapshot);
    setAutoRouteDeck(evaluateAutoRoutes(snapshot));
    setSelectedAutoRouteId(null);
  }, []);

  const setManualControlsWithPresetTracking = useCallback(
    (controls: EngineControls, options?: { refreshAutoRoutes?: boolean; selectedRouteId?: string | null }) => {
      const snapshot = cloneControls(controls);
      setManualControls(snapshot);
      setSelectedNavigatorPresetId(findMatchingPresetId(snapshot, presetList));

      if (options?.refreshAutoRoutes === false) {
        if (options?.selectedRouteId !== undefined) {
          setSelectedAutoRouteId(options.selectedRouteId);
        }
        return;
      }

      rebuildAutoRouteDeck(snapshot);
    },
    [presetList, rebuildAutoRouteDeck],
  );
  ...
  const updateManualControl = <K extends keyof EngineControls>(key: K, value: EngineControls[K]) => {
    ...
    setLastOptimization((previous) => ({
      ...previous,
      navigation: null,
    }));
    setManualControlsWithPresetTracking(nextControls);
  };

  const applyAutoRoute = (route: AutoRouteRecommendation) => {
    if (selectedAutoRouteId === route.id) {
      return;
    }

    setManualControlsWithPresetTracking(route.controls, {
      refreshAutoRoutes: false,
      selectedRouteId: route.id,
    });
    setLastOptimization((previous) => ({
      ...previous,
      navigation: buildOptimizationSummary(route),
    }));
  };
  ...
  const applyPreset = (presetId: string) => {
    ...
    if (selectedNavigatorPresetId === preset.id && findMatchingPresetId(manualControls, presetList) === preset.id) {
      return;
    }
    ...
    setManualControlsWithPresetTracking({
  ...
  const updateManualPracticalInput = (field: keyof PracticalInputs, value: number) => {
    const nextInputs = {
      ...manualPracticalInputs,
      [field]: value,
    };
    ...
    setManualControlsWithPresetTracking(applyPracticalInputsToControls(nextInputs, manualControls));
  };
  ...
  const updateDecisionTarget = (axis: 0 | 1 | 2, value: number) => {
    ...
    const currentTarget = (selectedDecisionOption.overrides?.target ?? selectedDecisionBase.target).slice() as Vector3;
    currentTarget[axis] = value;
  ...
  const autopilotDecisionSelection = () => {
    if (!selectedDecisionOption) {
      return;
    }

    const optimized = optimizeDecisionOption(selectedDecisionOption);
    updateDecisionOption(selectedDecisionOption.id, () => optimized.option);
    setLastOptimization((previous) => ({
      ...previous,
      decision: optimized.summary,
    }));
  };
  ...
  const updateIntentTarget = (axis: 0 | 1 | 2, value: number) => {
    setLastOptimization((previous) => ({
      ...previous,
      intent: null,
    }));
    const currentTarget = (intentScenario.overrides?.target ?? intentBase.target).slice() as Vector3;
    currentTarget[axis] = value;
  ...
  const autopilotIntent = () => {
    const optimized = optimizeIntentScenario(intentScenario);
    setIntentScenario(optimized.intent);
    setLastOptimization((previous) => ({
      ...previous,
      intent: optimized.summary,
    }));
  };

  const autopilotNavigation = () => {
    const route = winningAutoRoute ?? evaluateAutoRoutes(manualControls)[0];
    if (!route) {
      return;
    }

    applyAutoRoute(route);
  };
  ...
  const exportRuns = () => {
    ...
    link.href = url;
    link.download = "professional-launch-runs.json";
    link.click();
  ...
  const renderOptimizationSummary = (title: string, summary: OptimizationSummary | null) => {
    if (!summary) {
      return null;
    }
    ...
    return (
      <GlowBox color={P.gold}>
        <div><strong>{title}:</strong> {summary.routeName}</div>
        <div><strong>Rationale:</strong> {summary.rationale}</div>
        <div><strong>Optimized Target:</strong> {formatVector(summary.optimizedTarget)}</div>
        <div><strong>Δ Cost:</strong> {formatSignedValue(summary.deltas.chosenCost)}</div>
        <div><strong>Δ Risk:</strong> {formatSignedValue(summary.deltas.riskScore)}</div>
        <div><strong>Δ Probability:</strong> {formatSignedValue(summary.deltas.chosenProbability)}</div>
      </GlowBox>
    );
  };
  ...
  const renderDecisionControls = () => {
    ...
    return (
      <div style={{ background: P.panel, border: `1px solid ${P.border}`, borderRadius: 12, padding: 16 }}>
        ...
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${P.border}` }}>
          <div style={{ color: P.dim, fontSize: 12, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.08em" }}>Target Vector</div>
          <Knob label="Target X" value={overrideControls.target[0]} onChange={(value) => updateDecisionTarget(0, value)} min={-20} max={20} step={0.1} color={P.glow} />
          <Knob label="Target Y" value={overrideControls.target[1]} onChange={(value) => updateDecisionTarget(1, value)} min={-20} max={20} step={0.1} color={P.glow2} />
          <Knob label="Target Z" value={overrideControls.target[2]} onChange={(value) => updateDecisionTarget(2, value)} min={-20} max={20} step={0.1} color={P.glow3} />
        </div>

        <button
          onClick={autopilotDecisionSelection}
          style={{
  ...
  const renderIntentControls = () => {
    ...
    return (
      <div style={{ background: P.panel, border: `1px solid ${P.border}`, borderRadius: 12, padding: 16 }}>
        ...
        <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${P.border}` }}>
          <div style={{ color: P.dim, fontSize: 12, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.08em" }}>Target Vector</div>
          <Knob label="Target X" value={overrideControls.target[0]} onChange={(value) => updateIntentTarget(0, value)} min={-20} max={20} step={0.1} color={P.glow} />
          <Knob label="Target Y" value={overrideControls.target[1]} onChange={(value) => updateIntentTarget(1, value)} min={-20} max={20} step={0.1} color={P.glow2} />
          <Knob label="Target Z" value={overrideControls.target[2]} onChange={(value) => updateIntentTarget(2, value)} min={-20} max={20} step={0.1} color={P.glow3} />
        </div>

        <button
          onClick={autopilotIntent}
          style={{
            border: `1px solid ${P.gold}`,
            background: `${P.gold}14`,
            color: P.gold,
  ...
  const renderManualControls = (
    ...
    <div style={{ background: P.panel, border: `1px solid ${P.border}`, borderRadius: 12, padding: 16 }}>
      ...
      {options.showPracticalInputs && (
        <div style={{ marginBottom: 14, paddingBottom: 12, borderBottom: `1px solid ${P.border}` }}>
          <div style={{ color: P.dim, fontSize: 12, marginBottom: 8, textTransform: "uppercase", letterSpacing: "0.08em" }}>Practical Inputs</div>
          <Knob label="Alignment" value={manualPracticalInputs.alignment} onChange={(value) => updateManualPracticalInput("alignment", value)} min={0} max={1} color={P.glow3} />
          <Knob label="Complexity" value={manualPracticalInputs.complexity} onChange={(value) => updateManualPracticalInput("complexity", value)} min={0} max={1} color={P.glow} />
          <Knob label="Time Horizon" value={manualPracticalInputs.timeHorizon} onChange={(value) => updateManualPracticalInput("timeHorizon", value)} min={0} max={1} color={P.glow2} />
          <Knob label="Stability" value={manualPracticalInputs.stability} onChange={(value) => updateManualPracticalInput("stability", value)} min={0} max={1} color={P.green} />
        </div>
      )}
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 14 }}>
        ...
        <div>
          ...
          <div style={{ marginTop: 12 }}>
            <div style={{ color: P.dim, fontSize: 12, marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.06em" }}>Navigator Presets</div>
            <div style={{ color: selectedNavigatorPresetId ? P.glow : P.gold, fontSize: 11, marginBottom: 8 }}>
              {selectedNavigatorPresetId ? `Selected: ${presetList.find((preset) => preset.id === selectedNavigatorPresetId)?.name ?? "Preset"}` : "Selected: Custom"}
            </div>
            <div style={{ color: P.dim, fontSize: 11, marginBottom: 8, lineHeight: 1.6 }}>
              Presets are static starting baselines. They do not cycle. If you change any knob after selecting one, the state switches to Custom.
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div
                style={{
                  textAlign: "left",
                  border: `1px solid ${selectedNavigatorPresetId === null ? P.gold : P.border}`,
                  background: selectedNavigatorPresetId === null ? `${P.gold}12` : P.ink,
                  color: selectedNavigatorPresetId === null ? P.gold : P.dim,
                  padding: "8px 10px",
                  borderRadius: 8,
                  fontFamily: FONT,
                  fontSize: 12,
              ...
              {presetList.map((preset, index) => (
                <button
                  ...
                  style={{
                    textAlign: "left",
                    border: `1px solid ${selectedNavigatorPresetId === preset.id ? P.glow : P.border}`,
                    background: selectedNavigatorPresetId === preset.id ? `${P.glow}14` : P.ink,
                    color: selectedNavigatorPresetId === preset.id ? P.glow : P.text,
                    padding: "8px 10px",
                    borderRadius: 8,
                    cursor: "pointer",
                    fontFamily: FONT,
      ...
      {options.showOptimizeButton && (
        <>
          {options.optimizeNote && (
            <div style={{ color: P.dim, fontSize: 11, lineHeight: 1.6, marginTop: 12 }}>
              {options.optimizeNote}
            </div>
          )}
          <button
            onClick={autopilotNavigation}
            style={{
              border: `1px solid ${P.gold}`,
              background: `${P.gold}14`,
              color: P.gold,
          ...
          >
            {options.optimizeLabel ?? "Autopilot Optimize"}
          </button>
        </>
      )}
    </div>
  ...
  const renderModeSpecificBottom = () => {
      ...
      return (
        <div style={{ display: "grid", gridTemplateColumns: isCompact ? "1fr" : "minmax(0, 2fr) minmax(280px, 1fr)", gap: 20, marginTop: 20 }}>
          ...
          <div style={{ display: "grid", gap: 14 }}>
            {renderOptimizationSummary("Decision Optimization", lastOptimization.decision)}
            <EngineLog logs={logs} />
          </div>
        </div>
      );
    ...
    if (mode === "INTENT") {
      return (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20, marginTop: 20 }}>
          ...
          {renderOptimizationSummary("Intent Optimization", lastOptimization.intent)}
          <EngineLog logs={logs} />
        </div>
      );
    }
        ...
        <div style={{ display: "grid", gridTemplateColumns: isCompact ? "1fr" : "minmax(0, 2fr) minmax(300px, 1fr)", gap: 20, marginTop: 20 }}>
          <div>
            <GlowBox color={P.glow}>
              <div><strong>Autopilot Workflow:</strong> 1. Set a baseline on the right. 2. Review the generated route deck. 3. Apply one route or use the top-route button.</div>
              <div><strong>Important:</strong> The six cards below are route recommendations, not presets. They stay fixed until you change the baseline controls again.</div>
            </GlowBox>
            <div style={{ color: P.dim, fontSize: 12, margin: "10px 0 12px", textTransform: "uppercase", letterSpacing: "0.08em" }}>
              Generated Route Deck
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
              {autoRoutes.map((route) => {
                const winner = route.id === winningAutoRoute.id;
                const applied = route.id === selectedAutoRouteId;

                return (
                  <div
                    key={route.id}
                    style={{
                      border: `1px solid ${applied ? P.glow : winner ? P.green : P.border}`,
                      background: applied ? `${P.glow}12` : winner ? `${P.green}12` : P.panel,
                      color: P.text,
                      padding: 14,
                      borderRadius: 12,
                      boxShadow: "0 18px 40px rgba(0,0,0,0.18)",
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center", marginBottom: 8 }}>
                      ...
                      {applied ? (
                        <span style={{ color: P.glow, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em" }}>Applied</span>
                      ) : winner ? (
                        <span style={{ color: P.green, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em" }}>Top Rank</span>
                      ) : null}
                    </div>
                    ...
                    <button
                      ...
                      style={{
                        marginTop: 12,
                        border: `1px solid ${applied ? P.glow : winner ? P.green : P.glow}`,
                        background: applied ? `${P.glow}14` : winner ? `${P.green}14` : `${P.glow}12`,
                        color: applied ? P.glow : winner ? P.green : P.glow,
                        padding: "8px 10px",
                        borderRadius: 8,
                      ...
                      disabled={applied}
                    >
                      {applied ? "Applied to Controls" : "Apply Route"}
                    </button>
                  </div>
                );
              })}
          ...
          <div style={{ display: "grid", gap: 14 }}>
            <GlowBox color={P.gold}>
              <div><strong>Route Deck Baseline:</strong> {formatVector(autoRouteDeckBase.target)}</div>
              <div><strong>Active Navigator Target:</strong> {formatVector(manualControls.target)}</div>
            ...
            <GlowBox color={P.glow2}>
              <div><strong>How to use this tab:</strong> Choose a navigator preset or set your own controls, then apply one route card once. The route deck only refreshes when you change the inputs again.</div>
              <div><strong>Why the top route won:</strong> {winningAutoRoute.rationale}</div>
    ...
    if (mode === "NAVIGATION") {
      return (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20, marginTop: 20 }}>
          ...
          {renderOptimizationSummary("Navigation Optimization", lastOptimization.navigation)}
          <EngineLog logs={logs} />
        </div>
      );
    }
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20, marginTop: 20 }}>
        ...
        {renderOptimizationSummary("Research Optimization", lastOptimization.navigation)}
        <GlowBox color={P.gold}>
          <div><strong>Research Mapping:</strong> Guided inputs and raw controls stay synchronized here.</div>
          <div><strong>Alignment:</strong> {manualPracticalInputs.alignment.toFixed(3)}</div>
          <div><strong>Complexity:</strong> {manualPracticalInputs.complexity.toFixed(3)}</div>
          <div><strong>Time Horizon:</strong> {manualPracticalInputs.timeHorizon.toFixed(3)}</div>
          <div><strong>Stability:</strong> {manualPracticalInputs.stability.toFixed(3)}</div>
        </GlowBox>
        <GlowBox color={P.glow2}>
    ...
    >
      ...
      >
        ...
        {mode === "RESEARCH" ? (
          ...
          <GlowBox color={P.glow}>
            {mode === "AUTOPILOT"
              ? "Navigator presets on the right are fixed starting baselines. The six cards below are one-shot route recommendations generated from the current controls; they stay fixed until you change the inputs again."
        ...
        <div style={{ display: "grid", gridTemplateColumns: isCompact ? "1fr" : "minmax(0, 1fr) 360px", gap: 20, alignItems: "start", position: "relative", zIndex: 1 }}>
          ...
          <div>
            ...
            {mode === "NAVIGATION" &&
              renderManualControls("Navigation Controls", {
                showPracticalInputs: true,
                showOptimizeButton: true,
                optimizeNote: "This applies the current top-ranked route to the active navigation controls.",
              })}
            {mode === "AUTOPILOT" &&
              renderManualControls("Navigator Inputs", {
                showPracticalInputs: true,
                showOptimizeButton: true,
                optimizeLabel: "Apply Top Route",
                optimizeNote: "This applies the top-ranked route from the generated route deck below.",
              })}
            {mode === "RESEARCH" &&
              renderManualControls("Research Controls", {
                showPracticalInputs: true,
                showOptimizeButton: true,
                optimizeNote: "This applies the current top-ranked route directly to the research controls.",
              })}
          </div>
        </div>
