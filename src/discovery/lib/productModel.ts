...
export function inferPracticalInputsFromEngine(controls: EngineControls): PracticalInputs {
  return {
    alignment: clamp((controls.coherence + (controls.ethics + 1) / 2) / 2, 0, 1),
    complexity: clamp((controls.energy - 0.25) / 0.75, 0, 1),
    timeHorizon: clamp((controls.curvature - 0.2) / 0.65, 0, 1),
    stability: clamp(1 - controls.instability, 0, 1),
  };
}

export function applyPracticalInputsToControls(inputs: PracticalInputs, controls: EngineControls): EngineControls {
  const mapped = mapPracticalInputsToEngine(inputs, controls.eta);

  return {
    ...controls,
    energy: mapped.energy,
    curvature: mapped.curvature,
    coherence: mapped.coherence,
    ethics: mapped.ethics,
    instability: mapped.instability,
  };
}

function controlsEqual(left: EngineControls, right: EngineControls) {
  return (
    Math.abs(left.energy - right.energy) < CONTROL_EPSILON &&
    Math.abs(left.curvature - right.curvature) < CONTROL_EPSILON &&
    Math.abs(left.coherence - right.coherence) < CONTROL_EPSILON &&
    Math.abs(left.ethics - right.ethics) < CONTROL_EPSILON &&
    Math.abs(left.instability - right.instability) < CONTROL_EPSILON &&
    Math.abs(left.eta - right.eta) < CONTROL_EPSILON &&
    left.target.every((value, index) => Math.abs(value - right.target[index]) < CONTROL_EPSILON)
  );
}

export function findMatchingPresetId(controls: EngineControls, presetList: Preset[]): string | null {
  const match = presetList.find((preset) =>
    controlsEqual(controls, {
...
export function buildOptimizationSummary(route: AutoRouteRecommendation): OptimizationSummary {
  return {
    routeId: route.id,
    routeName: route.name,
    rationale: route.rationale,
    optimizedTarget: route.controls.target,
    optimizedControls: route.controls,
    deltas: route.deltas,
  };
}

export function optimizeDecisionOption(option: DecisionOption) {
  const mapped = mapPracticalInputsToEngine(option.inputs, option.overrides?.eta ?? DEFAULT_ETA);
  const baselineControls = applyEngineOverrides(mapped, option.overrides);
  const route = evaluateAutoRoutes(baselineControls)[0];
  ...
  return {
    option: {
      ...option,
      overrides: route.controls,
    },
    summary: buildOptimizationSummary(route),
  };
}

export function optimizeIntentScenario(intent: IntentScenario) {
  const mapped = mapPracticalInputsToEngine(intent.inputs, intent.overrides?.eta ?? DEFAULT_ETA);
  const baselineControls = applyEngineOverrides(mapped, intent.overrides);
  const route = evaluateAutoRoutes(baselineControls)[0];
...
export function optimizeManualControls(controls: EngineControls) {
  const route = evaluateAutoRoutes(controls)[0];
