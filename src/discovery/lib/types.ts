...
export type OptimizationSummary = {
  ...
  routeName: string;
  rationale: string;
  optimizedTarget: Vector3;
  optimizedControls: EngineControls;
  deltas: {
    foldScore: number;
    chosenCost: number;
    riskScore: number;
...
export type OptimizationMemory = {
  decision: OptimizationSummary | null;
  intent: OptimizationSummary | null;
  navigation: OptimizationSummary | null;
};
...
export type PersistedProductState = {
  mode: EngineMode;
  manualControls: EngineControls;
  selectedNavigatorPresetId: string | null;
  decisionOptions: DecisionOption[];
  selectedDecisionId: string;
  intentScenario: IntentScenario;
  lastOptimization: OptimizationMemory;
