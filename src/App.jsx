...
import { PRODUCT_STATE_STORAGE_KEY, deserializePersistedProductState, serializePersistedProductState } from "./discovery/lib/persistence.ts";
import { APP_BRAND, FEATURE_CARDS, MANUAL_SECTIONS, ABOUT_SECTIONS, LEGAL_DOCUMENTS, SETTINGS_SECTIONS } from "./launch/content.js";
...
import { DEFAULT_LAUNCH_DESTINATION, DEFAULT_LAUNCH_SECTION, SHELL_STATE_STORAGE_KEY, deserializeShellState, getDefaultMobileSettings, serializeShellState } from "./platform/settings";
import { loadStoredString, removeStoredString, saveStoredString } from "./platform/storage";
...
function UtilityButton({ label, active, onClick, color = "#0f172a", subtle = false }) {
  return (
    <button
      type="button"
      onClick={onClick}
      style={{
        ...
        padding: "10px 14px",
        borderRadius: 999,
        border: subtle ? "1px solid rgba(148,163,184,0.28)" : "1px solid transparent",
        background: active ? color : subtle ? "rgba(255,255,255,0.74)" : "rgba(255,255,255,0.92)",
        color: active ? "#fff" : "#0f172a",
        fontFamily: FONT,
        fontSize: 12,
        fontWeight: 600,
        cursor: "pointer",
        boxShadow: active ? "0 12px 34px rgba(15,23,42,0.18)" : "0 4px 18px rgba(15,23,42,0.06)",
        whiteSpace: "nowrap",
      }}
    >
      {label}
...
function LightCard({ children, padded = true, accent = "#ffffff" }) {
  return (
    <div
      style={{
        background: accent,
...
function RuntimeApp({ active, onActiveChange }) {
  ...
  return (
    <div style={{ color: P.text, fontFamily: FONT, position: "relative", overflowX: "hidden" }}>
      <div style={{ position: "relative", zIndex: 1 }}>
        <div style={{ marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: isMobile ? "stretch" : "baseline", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <div>
              <h1 style={{ margin: 0, fontSize: isMobile ? 24 : 22, fontWeight: 700, color: "#fff", fontFamily: "'Georgia', serif" }}>{APP_BRAND.appName}</h1>
              <span style={{ display: "inline-block", marginTop: 4, fontSize: 11, color: P.glow, letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 600 }}>
                {APP_BRAND.foundationName} · {APP_BRAND.intelligenceName} inside · {APP_BRAND.engineName} engine runtime
              </span>
            </div>
          </div>
          <div style={{ fontSize: 12, color: P.dim, marginTop: 4, maxWidth: 720, lineHeight: 1.6 }}>
            {"Umbrella runtime: the original Fold-Space Engine tabs sit alongside the bridge modes so the full app works tab by tab inside one grounded web and native surface. Grounded in MQGT\u2013SCF notation by C. M. Baird. "}
...
function renderBulletList(items, color = "#334155") {
  return (
    <ul style={{ margin: "8px 0 0", paddingLeft: 18, color, lineHeight: 1.7 }}>
      {items.map((item) => (
        <li key={item}>{item}</li>
      ))}
    </ul>
...
export default function App() {
  const { isMobile } = useViewport();
  const [activeDeviceId, setActiveDeviceId] = useState(DEFAULT_ACTIVE_DEVICE);
  const [activeSection, setActiveSection] = useState(DEFAULT_LAUNCH_SECTION);
  const [launchDestination, setLaunchDestination] = useState(DEFAULT_LAUNCH_DESTINATION);
  const [settings, setSettings] = useState(() => getDefaultMobileSettings());
  const [acceptedTermsVersion, setAcceptedTermsVersion] = useState(null);
  const [acceptedPrivacyVersion, setAcceptedPrivacyVersion] = useState(null);
  ...
  useEffect(() => {
    loadStoredString(SHELL_STATE_STORAGE_KEY).then((raw) => {
      ...
      if (restored) {
        ...
        setActiveDeviceId(restored.lastAppTab);
        setLaunchDestination(restored.launchDestination);
        setAcceptedTermsVersion(restored.acceptedTermsVersion);
        setAcceptedPrivacyVersion(restored.acceptedPrivacyVersion);
        setActiveSection(
          restored.launchDestination === "RESUME_APP" && restored.settings.onboardingCompleted
            ? "APP"
  ...
  useEffect(() => {
    ...
    void saveStoredString(SHELL_STATE_STORAGE_KEY, serializeShellState(payload));
  }, [activeDeviceId, activeSection, acceptedPrivacyVersion, acceptedTermsVersion, hydrated, launchDestination, settings]);
  ...
  useEffect(() => {
    ...
    const register = async () => {
      removeHandler = await registerNativeBackHandler(async () => {
        const action = resolveShellBackAction({
          activeSection,
          activeDeviceId,
          defaultDeviceId: DEFAULT_ACTIVE_DEVICE,
          settingsOpen: activeSection === "SETTINGS",
          onboardingVisible: activeSection === "HOME" && !settings.onboardingCompleted,
          hasDiscoveryBackAction: false,
        });

        if (action === "close-settings" || action === "go-home") {
        ...
        if (action === "go-default-tab") {
          setActiveDeviceId(DEFAULT_ACTIVE_DEVICE);
          setActiveSection("APP");
          return true;
        }

        if (action === "delegate-discovery") {
          return true;
        }
  ...
  }, [activeDeviceId, activeSection, hydrated, settings.onboardingCompleted]);
  ...
  const acknowledgeAndStart = useCallback(() => {
    setSettings((previous) => ({ ...previous, onboardingCompleted: true }));
    setAcceptedTermsVersion((current) => current ?? APP_BRAND.legalVersion);
    setAcceptedPrivacyVersion((current) => current ?? APP_BRAND.legalVersion);
    setActiveSection("APP");
    setStatusMessage("Launch guidance acknowledged. Runtime ready.");
  }, []);

  const exportLocalBundle = useCallback(async () => {
    const productRaw = await loadStoredString(PRODUCT_STATE_STORAGE_KEY);
    const productState = deserializePersistedProductState(productRaw);
    const payload = {
      app: APP_BRAND.appName,
      foundation: APP_BRAND.foundationName,
    ...
    await exportJsonArtifact(
      buildJsonExportArtifact("professional-launch-app-bundle.json", JSON.stringify(payload, null, 2)),
    );
    setStatusMessage("Local app bundle exported.");
  }, [activeDeviceId, activeSection, acceptedPrivacyVersion, acceptedTermsVersion, launchDestination, settings]);

  const handleImportArchive = useCallback(async (event) => {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        setStatusMessage(`Raw run archive loaded for review: ${parsed.length} entries.`);
        event.target.value = "";
        return;
      }

      const restoredShell = parsed?.shellState
        ? deserializeShellState(JSON.stringify(parsed.shellState), DEFAULT_ACTIVE_DEVICE)
        : null;
      const restoredProduct = parsed?.productState
        ? deserializePersistedProductState(JSON.stringify(parsed.productState))
        : null;

      if (!restoredShell && !restoredProduct) {
        throw new Error("Unsupported archive format");
      }

      if (restoredShell) {
        setActiveSection(restoredShell.activeSection);
        setActiveDeviceId(restoredShell.lastAppTab);
        setLaunchDestination(restoredShell.launchDestination);
        setAcceptedTermsVersion(restoredShell.acceptedTermsVersion);
        setAcceptedPrivacyVersion(restoredShell.acceptedPrivacyVersion);
        setSettings(restoredShell.settings);
        await saveStoredString(SHELL_STATE_STORAGE_KEY, serializeShellState(restoredShell));
      }

      if (restoredProduct) {
        await saveStoredString(PRODUCT_STATE_STORAGE_KEY, serializePersistedProductState(restoredProduct));
      }

      setStatusMessage("Archive imported. Runtime state refreshed.");
    } catch (error) {
      setStatusMessage(`Import failed: ${error instanceof Error ? error.message : "Unknown archive error"}`);
    } finally {
      event.target.value = "";
    }
  }, []);
  ...
  const resetLocalState = useCallback(async () => {
    await removeStoredString(SHELL_STATE_STORAGE_KEY);
    await removeStoredString(PRODUCT_STATE_STORAGE_KEY);
    setActiveSection(DEFAULT_LAUNCH_SECTION);
    setActiveDeviceId(DEFAULT_ACTIVE_DEVICE);
    setLaunchDestination(DEFAULT_LAUNCH_DESTINATION);
    setAcceptedTermsVersion(null);
    setAcceptedPrivacyVersion(null);
    setSettings(getDefaultMobileSettings());
    setStatusMessage("Local app state reset.");
  }, []);

  const shellBackground = activeSection === "APP" ? P.void : "#eef3f9";
  const shellText = activeSection === "APP" ? P.text : "#0f172a";
  const onboardingVisible = activeSection === "HOME" && !settings.onboardingCompleted;

  return (
    <div style={{ minHeight: "100vh", background: shellBackground, color: shellText, fontFamily: FONT, transition: "background 0.25s ease" }}>
      <div
        style={{
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", gap: 14, alignItems: "center", flexWrap: "wrap", marginBottom: 18 }}>
          <div>
            <div style={{ color: activeSection === "APP" ? P.glow : "#2563eb", textTransform: "uppercase", letterSpacing: "0.16em", fontSize: 11, fontWeight: 700 }}>
              {APP_BRAND.foundationName}
            </div>
            <div style={{ fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: isMobile ? 28 : 34, fontWeight: 700 }}>
              {APP_BRAND.appName}
            </div>
            <div style={{ marginTop: 4, color: activeSection === "APP" ? P.dim : "#475569", fontSize: 13, lineHeight: 1.6, maxWidth: 760 }}>
              {APP_BRAND.tagline}
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: isMobile ? "nowrap" : "wrap", overflowX: isMobile ? "auto" : "visible", paddingBottom: 4 }}>
            {[
              ["HOME", "Home"],
              ["APP", "Return to App"],
              ["MANUAL", "Manual"],
              ...
              <UtilityButton
                ...
                color={activeSection === "APP" ? P.glow2 : "#0f172a"}
                subtle={activeSection !== sectionId}
              />
            ))}
          </div>
        </div>
        ...
        {statusMessage ? (
          <div
            style={{
              ...
              padding: "12px 14px",
              borderRadius: 16,
              background: activeSection === "APP" ? "rgba(19,22,36,0.88)" : "rgba(255,255,255,0.9)",
              border: activeSection === "APP" ? `1px solid ${P.border}` : "1px solid rgba(148,163,184,0.22)",
              color: activeSection === "APP" ? P.text : "#0f172a",
            }}
          >
            {statusMessage}
          </div>
        ...
        {activeSection === "HOME" ? (
          <div style={{ display: "grid", gap: 18 }}>
            <LightCard accent="linear-gradient(180deg, rgba(255,255,255,0.98), rgba(241,245,249,0.95))">
              <div style={{ display: "grid", gap: 18, gridTemplateColumns: isMobile ? "1fr" : "minmax(0, 1.5fr) minmax(280px, 0.9fr)" }}>
                <div>
                  <div style={{ color: "#2563eb", textTransform: "uppercase", letterSpacing: "0.16em", fontSize: 11, fontWeight: 700, marginBottom: 10 }}>
                    Overall App
                  </div>
                  <h2 style={{ margin: 0, fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: isMobile ? 30 : 40, lineHeight: 1.08, color: "#0f172a" }}>
                    {APP_BRAND.foundationName}
                  </h2>
                  <div style={{ marginTop: 12, color: "#334155", fontSize: 16, lineHeight: 1.7 }}>
                    {APP_BRAND.summary}
                  </div>
                  <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 20 }}>
                    <UtilityButton label="Start" active={false} onClick={acknowledgeAndStart} color="#2563eb" />
                    <UtilityButton label="Manual" active={false} onClick={() => openSection("MANUAL")} />
                    <UtilityButton label="About" active={false} onClick={() => openSection("ABOUT")} />
                    <UtilityButton label="Legal" active={false} onClick={() => openSection("LEGAL")} />
                    <UtilityButton label="Settings" active={false} onClick={() => openSection("SETTINGS")} />
                  </div>
                </div>
                <LightCard accent="rgba(248,250,252,0.9)">
                  <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 20, fontWeight: 700, marginBottom: 10 }}>
                    Internal Architecture
                  </div>
                  <div style={{ display: "grid", gap: 10 }}>
                    {[
                      ["Foundation", APP_BRAND.foundationName],
                      ["ASI Layer", APP_BRAND.intelligenceName],
                      ["Engine", APP_BRAND.engineName],
                    ].map(([label, value]) => (
                      <div key={label} style={{ padding: "14px 16px", borderRadius: 16, background: "#ffffff", border: "1px solid rgba(148,163,184,0.16)" }}>
                        <div style={{ color: "#64748b", textTransform: "uppercase", fontSize: 11, letterSpacing: "0.12em", marginBottom: 4 }}>{label}</div>
            ...
            {onboardingVisible ? (
              <LightCard accent="#fff7ed">
                <div style={{ color: "#9a3412", textTransform: "uppercase", letterSpacing: "0.12em", fontSize: 11, fontWeight: 700, marginBottom: 8 }}>
                  First Run Guidance
                </div>
                <div style={{ color: "#7c2d12", lineHeight: 1.7 }}>
                  Professional Launch is a software simulation and interface. Review the Manual, About, Privacy Policy, Terms of Use, and User Agreement before relying on the runtime in a serious workflow.
                </div>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
                  <UtilityButton label="Review Legal" active={false} onClick={() => openSection("LEGAL")} color="#c2410c" />
                  <UtilityButton label="Acknowledge and Start" active={false} onClick={acknowledgeAndStart} color="#ea580c" />
                </div>
              </LightCard>
            ) : null}
            ...
            <div style={{ display: "grid", gap: 16, gridTemplateColumns: isMobile ? "1fr" : "repeat(2, minmax(0, 1fr))" }}>
              {FEATURE_CARDS.map((card) => (
                <LightCard key={card.title}>
                  <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 21, fontWeight: 700, marginBottom: 8 }}>
                    {card.title}
                  </div>
                  <div style={{ color: "#475569", lineHeight: 1.8 }}>{card.body}</div>
                </LightCard>
              ))}
            </div>
          </div>
        ...
        {activeSection === "APP" ? (
          <div style={{ padding: isMobile ? 0 : 6 }}>
            <RuntimeApp active={activeDeviceId} onActiveChange={setActiveDeviceId} />
          </div>
        ) : null}
        ...
        {activeSection === "MANUAL" ? (
          <div style={{ display: "grid", gap: 16 }}>
            {MANUAL_SECTIONS.map((section) => (
              <LightCard key={section.id}>
                <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 24, fontWeight: 700, marginBottom: 8 }}>
                  {section.title}
                </div>
                {section.body.map((paragraph) => (
                  <p key={paragraph} style={{ margin: "0 0 12px", color: "#475569", lineHeight: 1.8 }}>
                    {paragraph}
                  </p>
                ))}
              </LightCard>
        ...
        {activeSection === "ABOUT" ? (
          <div style={{ display: "grid", gap: 16 }}>
            <LightCard>
              <div style={{ color: "#2563eb", textTransform: "uppercase", letterSpacing: "0.14em", fontSize: 11, fontWeight: 700, marginBottom: 8 }}>
                About
              </div>
              <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 30, fontWeight: 700, marginBottom: 10 }}>
                {APP_BRAND.appName}
              </div>
              <div style={{ color: "#334155", lineHeight: 1.8 }}>{APP_BRAND.summary}</div>
            </LightCard>
            {ABOUT_SECTIONS.map((section) => (
              <LightCard key={section.id}>
                <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 22, fontWeight: 700, marginBottom: 8 }}>
                  {section.title}
                </div>
                <div style={{ color: "#475569", lineHeight: 1.8 }}>{section.body}</div>
              </LightCard>
            ))}
            <LightCard>
              <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 22, fontWeight: 700, marginBottom: 8 }}>
              ...
              <div style={{ color: "#475569", lineHeight: 1.8 }}>
                Version {APP_BRAND.version} · Legal set {APP_BRAND.legalVersion}
              </div>
              <div style={{ color: "#475569", lineHeight: 1.8, marginTop: 8 }}>
                Support: <a href={APP_BRAND.supportUrl} target="_blank" rel="noreferrer" style={{ color: "#2563eb" }}>{APP_BRAND.supportUrl}</a>
              </div>
            </LightCard>
          </div>
        ) : null}
        ...
        {activeSection === "LEGAL" ? (
          <div style={{ display: "grid", gap: 16 }}>
            {LEGAL_DOCUMENTS.map((document) => (
              <LightCard key={document.id}>
                <div style={{ color: "#2563eb", textTransform: "uppercase", letterSpacing: "0.14em", fontSize: 11, fontWeight: 700, marginBottom: 8 }}>
                  {document.title}
                </div>
                <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 26, fontWeight: 700, marginBottom: 10 }}>
                  {document.summary}
                </div>
            ...
            <LightCard>
              ...
              {renderBulletList([APP_BRAND.privacyUrl, APP_BRAND.termsUrl, APP_BRAND.supportPageUrl])}
            </LightCard>
          </div>
        ) : null}
        ...
        {activeSection === "SETTINGS" ? (
          <div style={{ display: "grid", gap: 16 }}>
            <LightCard>
              ...
              <div style={{ color: "#475569", lineHeight: 1.8 }}>
                Manage accessibility, effects, launch behavior, local data, and public trust surfaces from one place.
              </div>
            </LightCard>

            <LightCard>
              <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 22, fontWeight: 700, marginBottom: 12 }}>
                Experience
              </div>
              <div style={{ display: "grid", gap: 12 }}>
                <label style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, color: "#334155" }}>
                  <span>Launch destination</span>
                  <select
                    value={launchDestination}
                    onChange={(event) => setLaunchDestination(event.target.value)}
                    style={{ padding: "10px 12px", borderRadius: 12, border: "1px solid rgba(148,163,184,0.24)", background: "#fff", fontFamily: FONT }}
                  >
                    <option value="HOME">Home</option>
                    <option value="RESUME_APP">Resume App</option>
                  </select>
                </label>
                <label style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, color: "#334155" }}>
                  <span>Reset onboarding acknowledgment</span>
                  <button
                    type="button"
                    onClick={() => setSettings((previous) => ({ ...previous, onboardingCompleted: false }))}
                    style={{ padding: "10px 12px", borderRadius: 12, border: "1px solid rgba(148,163,184,0.24)", background: "#fff", fontFamily: FONT, cursor: "pointer" }}
                  >
                    Reset
                  </button>
                </label>
            ...
            <LightCard>
              <div style={{ color: "#0f172a", fontFamily: "'SF Pro Display', 'Helvetica Neue', Arial, sans-serif", fontSize: 22, fontWeight: 700, marginBottom: 12 }}>
                Accessibility and Device Effects
              </div>
              <div style={{ display: "grid", gap: 12 }}>
                {[
                  ["safeMode", "Safe Mode"],
                  ...
                  ["brightnessEffectsEnabled", "Brightness Effects"],
                ].map(([key, label]) => (
                  <label key={key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, color: "#334155" }}>
                    <span>{label}</span>
                    <input
                      type="checkbox"
            ...
            <LightCard>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <UtilityButton label="Export Data / Logs" active={false} onClick={() => void exportLocalBundle()} color="#2563eb" />
                <UtilityButton label="Import Archive" active={false} onClick={() => fileInputRef.current?.click()} />
                <UtilityButton label="Reset Local App State" active={false} onClick={() => void resetLocalState()} color="#b91c1c" />
              </div>
              <input ref={fileInputRef} type="file" accept="application/json" onChange={(event) => void handleImportArchive(event)} style={{ display: "none" }} />
            </LightCard>
            ...
            <LightCard>
              ...
              {SETTINGS_SECTIONS.map((section) => (
                <div key={section.title} style={{ padding: "12px 0", borderTop: "1px solid rgba(148,163,184,0.16)" }}>
                  <div style={{ color: "#0f172a", fontWeight: 700 }}>{section.title}</div>
                  {renderBulletList(section.items)}
                </div>
              ))}
              <div style={{ marginTop: 10, color: "#475569", lineHeight: 1.8 }}>
                Accepted Terms version: {acceptedTermsVersion ?? "Not yet acknowledged"}<br />
                Accepted Privacy version: {acceptedPrivacyVersion ?? "Not yet acknowledged"}
              </div>
            </LightCard>
          </div>
        ) : null}
